	...
	*/
			...
			) 
	{
		...
		if (!IMPORT_RADIUS) { 	/* case R' = const */
			...
			import_network_radius(R_,Ri_, ist, mf_datav_);;
		}
...
//! Get the radius at a given branch
	inline scalar_type Ri  (size_type i) { return Ri_[i];  } const
...
//! Get the vessel bed permeability at a given dof
	inline scalar_type kvi (size_type i) { return kvi_[i]; } const
